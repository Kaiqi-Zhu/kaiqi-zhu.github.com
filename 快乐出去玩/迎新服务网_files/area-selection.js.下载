(function($){
	...
	var AreaSelection = function (el, options) {
        ...
        if (options["defaultValue"] != ""){
        	this.$el.data("data-area",options["defaultValue"]);
        }
        this.init(options);
    };
	...
	AreaSelection.prototype.init=function(options){
		...
		$this.after('<i class="fa fa-times input-remove" aria-hidden="true"></i>');
		$this.focus(function(){
			var $node = $(this);
	        ...
	        $('.area-selection').offset(offset);
	        $('.area-selection').css({
	            'top':function () {
	                return parseFloat(offset.top || 0) + eh; 
	            },
	            'width':ew
	        }).show();
	...
	/*	$('.area-selection').on('mouseenter',function(){
			$('.area-selection').data("enter",true);
		}).on('mouseleave',function(){
			$('.area-selection').removeData("enter");
		});*/
		
		/*$('.area-selection').on('outerClick',function(e){
			//此处是写死了判断的元素，建议插件重写
			if(!$("#hkszdmc").is(":focus") && !$("#jgmc").is(":focus")) {
				$('.area-selection').hide().css({'top':0,'left':0});
			}
			//var enter = $('.area-selection').data("enter");
			//if(enter){
    ...
    $(document).off('click','.input-group-input .input-remove').on('click','.input-group-input .input-remove',function(){
    	$(this).prev().val('');
		$(this).next().val('');
	});
}(jQuery));